...
export class VerificationService {
    ...
    private readonly validators: Record<NotificationChannel, (address: string) => boolean> = {
        [NotificationChannel.Phone]: (address: string) => parsePhoneNumber(address, 'RU').isValid(),
        [NotificationChannel.Email]: (address: string) => validator.isEmail(address, { allow_utf8_local_part: true }),
        [NotificationChannel.Telegram]: (address: string) => validator.isInt(address, { min: 1 }),
    };
    private readonly sanitizers: Record<NotificationChannel, (address: string) => string> = {
        [NotificationChannel.Phone]: (address: string) => parsePhoneNumber(address, 'RU').format('E.164'),
        [NotificationChannel.Email]: (address: string) => validator.normalizeEmail(address) as string,
        [NotificationChannel.Telegram]: (address: string) => validator.toInt(address).toFixed(0),
    }
    ...
    async verifyCode<T>(requestId: string, code: string): Promise<T> {
        const request = await this.storageService.get<VerifyRequest<T>>(this.keyFor(requestId));
        if (!request) {
