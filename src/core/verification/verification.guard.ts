import { BadRequestException, CanActivate, ExecutionContext, Injectable, mixin } from '@nestjs/common';
...
export const VerficationGuard = <T>(
  ...
  class VerificationGuardMixin implements CanActivate {
    ...
    async canActivate(
      ...
      if (!(requestIdField in request.body && confirmCodeField in request.body)) {
        throw new BadRequestException('Не указан ИД запроса или код подтверждения');
      }

      const requestId = request.body[requestIdField];
      const confirmCode = request.body[confirmCodeField];

      const data = await this.verificationService.verifyCode<T>(requestId, confirmCode);
      if (!data) {
        throw new BadRequestException('Код не действителен, попробуйте ещё раз');
      }
